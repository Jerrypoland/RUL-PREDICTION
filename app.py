...
if uploaded_file is not None:
    ...
    st.write("Running GridSearchCV to find the best parameters...")
    ...
    st.subheader("Evaluation Metrics for Random Forest Regression:")
    st.write(f"MSE: {mse}")
    st.write(f"MAE: {mae}")
    ...
    st.subheader("Actual vs. Predicted RUL (Random Forest)")
    ...
    # Function to get user input through Streamlit
    def get_user_input():
        st.sidebar.subheader("Input your battery features")
        Cycle_Index = st.sidebar.number_input("Cycle Index", min_value=0.0)
        ...
        Max_Voltage_Discharge = st.sidebar.number_input("Max. Voltage Discharge (V)", min_value=0.0)
        Min_Voltage_Charge = st.sidebar.number_input("Min. Voltage Charge (V)", min_value=0.0)
        Time = st.sidebar.number_input("Time at 4.15V (s)", min_value=0.0)
        Time_constant_current = st.sidebar.number_input("Time constant current (s)", min_value=0.0)
        Charging_time = st.sidebar.number_input("Charging time (s)", min_value=0.0)
    ...
    if user_features:
        ...
        st.write(f"The Predicted Remaining Useful Life (RUL) of your battery is: **{predicted_rul:.2f}**")
